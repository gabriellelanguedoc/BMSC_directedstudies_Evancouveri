...
# distribution of growth between treatments 
growth_sal <- tube_growth %>%
  select(treatment,total_growth_DS) %>%
  filter(treatment == 'low_salinity')

growth_heat <- tube_growth %>%
  select(treatment,total_growth_DS) %>%
  filter(treatment == 'high_temp')

growth_cont <- tube_growth %>%
  select(treatment,total_growth_DS) %>%
  filter(treatment == 'control')


shapiro.test(growth_sal$total_growth_DS) # pvalue = 0.5244
shapiro.test(growth_heat$total_growth_DS) # pvalue = 0.1149
shapiro.test(growth_cont$total_growth_DS) # pvalue = 0.001047

# OR distribution of growth amoung all our samples

shapiro.test(tube_growth$total_growth_DS) # p value = 7.081e-05
